...
class _MyGoogleMapState extends State<MyGoogleMap> {
          ...
          if (snapshot.connectionState == ConnectionState.done) {
            var googleMap = GoogleMap(
              initialCameraPosition: CameraPosition(
                  target: snapshot.data['location'] ?? lastCenter!, zoom: 15.0),
              ...
              onTap: widget.onTapPlaceMarker ? handleTap : null,
            );
  ...
  Future<LatLng?> getLocation() async {
    ...
    if (lastLat == null && lastLng == null) {
      lastCenter = LatLng(47.4733413, 19.0597962);
    } else {
      lastCenter = LatLng(lastLat!, lastLng!);
    }
  ...
  Future<dynamic> buildGoogleMap() async {
    ...
    return {'location': location, 'markers': _markers};
  }
}
